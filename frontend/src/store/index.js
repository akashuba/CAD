	...
	mutations: {
		...
		[mutations.REMOVE_CURRENT_EXHAUST](state) {
			if (state.exhaustCount === 1) {
				...
				const {
					[state.currentExhaustName]: currentExhaust,
					...rest
				} = state.ventUnitsExhaust;
		...
		[mutations.CREATE_SUPPLY_UNIT](state, payload) {
			...
			const newCurrentSupplyConfig = payload?.template
				? state.ventUnitsSupply[payload?.template]
				: defaultSupplyUnit;
		...
		[mutations.CREATE_EXHAUST_UNIT](state, payload) {
			...
			const newCurrentExhaustConfig = payload?.template
				? state.ventUnitsExhaust[payload?.template]
				: defaultExhaustUnit;
		...
		[mutations.UPLOAD_XML](state, payload) {
		...
		[mutations.RESET_CONFIG](state) {
			state = Object.assign(state, initialConfig)
		},
	},
	actions: {},
	modules: {},
